...
> 本文档记录 CareerCrew 独立 Web 前端的完整实施现状（2026-08-09 更新）。
...
**约束**：
- 保留现有 `careercrew chat` CLI 和 Streamlit Dashboard 不动
...
└── routers/{__init__,chat,interview,resume,consult,data}.py
web/                        # React 前端(独立 package.json,Vite 工程)
tests/api/                  # API 测试(TestClient + FakeRuntime 注入), 26 个
...
**核心决策**:重组件(llm/embedding/store/reranker/HybridSearch/user_model)进程级单例；**agent 与 JobCycle 按会话(thread_id)新建**；**情景记忆按 thread 分文件**(`data/transcripts/{user_id}/{thread_id}.jsonl`)。

- `_ensure_heavy()`: 复用 `careercrew_cli/app.py` 的 `_build_job_cycle` 组装逻辑(去 Renderer 化): create_embedding / create_vector_store / create_llm(max_tokens=1024) / create_reranker / HybridSearch;`store.count()==0` 时 IngestionPipeline 入库 KB;UserModelStore + TraceRecorder。捕获 Milvus `DataDirLockedError` → 抛 `RuntimeInitError`(503+清理指引)
...
- `run_match_stream(thread_id, user_id, intent, cb)`: 跑匹配 + **存 user_message/agent_response 到 episodic** + **首条消息时 LLM 生成 thread_title**
...
- 直通: `score_answer` / `record_interview_qa` / `read_image` / `load_document` / `consult_stream`
...
| GET | /api/traces | ?limit | 否 | 复用 get_traces() |
...
| POST | /api/resume/upload | multipart file | 否 | 图片→read_image;txt/md→MarkdownLoader;pdf/doc→MarkItDownLoader;**markitdown 需 [pdf] extra**;>200k 截断;解析失败返回 doc_type=error 不崩 |
...
- **页面**:
  ...
  - **DataPage**: 画像(**可编辑**,PUT /api/profile,空值清空)/记忆(格式化展示,所有线程)/轨迹(**倒序**)
- **关键组件**:
...
- `tests/api/`: 26 个测试(TestClient + FakeRuntime 注入),`@pytest.mark.web`
- 全量 `pytest tests`: 201 个(175 旧 + 26 新 API)零回归
- FakeRuntime duck-types CareerCrewRuntime: new_job_matcher/new_resume_advisor/new_interviewer/new_consult_agent/score_answer/record_interview_qa/read_image/load_document/run_match_stream/run_resume_stream/health_info/get_cycle/llm
