...
/** 设置页区块定义（侧边栏导航与内容页共用）。 */
export const SETTINGS_SECTIONS: { key: string; label: string; desc: string; icon: ComponentType<{ className?: string; strokeWidth?: number }> }[] = [
  { key: "profile", label: "能力画像", desc: "方向、技能、求职偏好与目标公司", icon: User },
