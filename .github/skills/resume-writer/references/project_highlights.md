...
- **锚点**：`DEV_SPEC.md` §3.1、`careercrew_core/supervisor/`、`careercrew_core/agents/`。
...
- **锚点**：`DEV_SPEC.md` §3.2、`careercrew_ai/react/react_loop.py`。
...
- **锚点**：`DEV_SPEC.md` §3.3、`careercrew_core/memory/`。
...
- **锚点**：`DEV_SPEC.md` §3.5/§3.7、`MODULAR-RAG-MCP-SERVER/src/libs/vector_store/milvus_store.py`。
...
- **锚点**：`DEV_SPEC.md` §3.4、`careercrew_core/tools/registry.py`。
...
- **锚点**：`DEV_SPEC.md` §3.8、`careercrew_core/supervisor/hitl.py`。
...
- **锚点**：`DEV_SPEC.md` §3.9、`careercrew_cli/workflow/job_cycle.py`。

## 亮点 8：全链路可观测 + 评估闭环
- **核心说法**：复用 MODULAR-RAG 全链路 trace（JSON Lines）+ Streamlit Dashboard；答案级评估（简历匹配度/面试题质量，复用 Ragas）+ 业务级评估（转化率/通过率/offer）。
- **话术方向**：agent 系统的黑盒问题用 trace 破解；不依赖 LangSmith（本地优先）；轨迹级评估（高级，LLM-as-judge + 黄金回放）。
- **量化角度**：trace 覆盖 4 类（agent_loop/hitl/memory_op/compaction）、答案级+业务级双维度。
- **锚点**：`DEV_SPEC.md` §3.10/§3.11、`careercrew_ui/dashboard/`。

## 亮点 9：本地优先（零外部服务）
- **核心说法**：Milvus Lite 嵌入式 + SQLite checkpointer + JSONL transcripts + User Model JSON，LLM 可插拔复用 llm_factory，零外部服务即可跑通 MVP。
- **话术方向**：本地优先设计哲学；零依赖部署（pip install 即跑）；可扩展（checkpointer 换 Postgres、Milvus 换 Docker）。
- **量化角度**：0 外部服务、pip install 即用、Chroma 兜底。
- **锚点**：`DEV_SPEC.md` §2、`config/settings.yaml`。
...
- **锚点**：`.github/skills/`、`DEV_SPEC.md` §6 排期。
